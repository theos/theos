...
_THEOS_PLATFORM_DPKG_DEB ?= $(shell PATH=$$PATH:$(THEOS_BIN_PATH) which dm.pl)
...
internal-package::
	$(ECHO_NOTHING)COPYFILE_DISABLE=1 $(FAKEROOT) -r "$(if $(findstring .pl,$(_THEOS_PLATFORM_DPKG_DEB)),$(PERL) )$(_THEOS_PLATFORM_DPKG_DEB)" -Z$(_THEOS_PLATFORM_DPKG_DEB_COMPRESSION) -b "$(THEOS_STAGING_DIR)" "$(_THEOS_DEB_PACKAGE_FILENAME)"$(ECHO_END)
