...
$(error Missing private frameworks for this target. Expected private frameworks at "$(TARGET_PRIVATE_FRAMEWORK_PATH)")
	...
	_THEOS_USING_GENERATED_PRIVATE_FRAMEWORKS := $(_THEOS_TRUE)
	TARGET_PRIVATE_FRAMEWORK_PATH = $(THEOS_OBJ_DIR_NAME)/$(THEOS_TARGET_NAME)/PrivateFrameworks
endif
...
ifeq ($(_THEOS_USING_GENERATED_PRIVATE_FRAMEWORKS),$(_THEOS_TRUE))
$(TARGET_PRIVATE_FRAMEWORK_PATH):
	$(ECHO_NOTHING)"$(THEOS_BIN_PATH)/generate_tbds.sh" "$(TARGET_PRIVATE_FRAMEWORK_FALLBACK_SOURCE_PATH)" "$(TARGET_PRIVATE_FRAMEWORK_PATH)"$(ECHO_END)

before-$(THEOS_CURRENT_INSTANCE)-all:: $(TARGET_PRIVATE_FRAMEWORK_PATH)
endif
