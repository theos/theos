...
ifeq ($(TARGET_CODESIGN),)
	...
	_THEOS_ENTITLEMENTS_FILE := $(or $($(THEOS_CURRENT_INSTANCE)_CODESIGN_ENTITLEMENTS),$(THEOS_CURRENT_INSTANCE).entitlements)
	ifneq ($(call __exists,$(_THEOS_ENTITLEMENTS_FILE)),$(_THEOS_TRUE))
		_THEOS_ENTITLEMENTS_FILE :=
	endif

	ifeq ($(findstring $(TARGET_CODESIGN),ldid),ldid)
		_THEOS_CODESIGN_COMMANDLINE = $(TARGET_CODESIGN) $(TARGET_CODESIGN_FLAGS) -S$(_THEOS_ENTITLEMENTS_FILE)
	else ifeq ($(TARGET_CODESIGN),codesign)
		_THEOS_CODESIGN_COMMANDLINE = $(TARGET_CODESIGN) $(TARGET_CODESIGN_FLAGS) --sign $(or $($(THEOS_CURRENT_INSTANCE)_CODESIGN_IDENTITY),-) $(if $(_THEOS_ENTITLEMENTS_FILE),--entitlements $(_THEOS_ENTITLEMENTS_FILE))
	else
		_THEOS_CODESIGN_COMMANDLINE = $(TARGET_CODESIGN) $(TARGET_CODESIGN_FLAGS)
	endif
endif
