...
THEOS_SHARED_BUNDLE_FRAMEWORK_PATH = $(_THEOS_SHARED_BUNDLE_STAGE_PATH)/$(_THEOS_TARGET_BUNDLE_FRAMEWORK_SUBDIRECTORY)
...
THEOS_SHARED_BUNDLE_FRAMEWORK_PATH = $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_FRAMEWORK_SUBDIRECTORY)
THEOS_SHARED_BUNDLE_HEADERS_PATH = $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_HEADERS_SUBDIRECTORY)
endif

$(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_BINARY_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_RESOURCE_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_FRAMEWORK_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_INFO_PLIST_SUBDIRECTORY)::
...
_FRAMEWORK_DIRS := $(or $($(THEOS_CURRENT_INSTANCE)_BUNDLE_FRAMEWORK_DIRS),$($(THEOS_CURRENT_INSTANCE)_FRAMEWORK_DIRS))
...
ifeq ($(_FRAMEWORK_DIRS),)
ifeq ($(call __exists,Frameworks),$(_THEOS_TRUE))
_FRAMEWORK_DIRS := Frameworks
else
_FRAMEWORK_DIRS :=
endif
endif

shared-instance-bundle-all:: $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_BINARY_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_RESOURCE_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_FRAMEWORK_SUBDIRECTORY) $(_THEOS_SHARED_BUNDLE_BUILD_PATH)/$(_THEOS_TARGET_BUNDLE_INFO_PLIST_SUBDIRECTORY)
...
ifneq ($(_FRAMEWORK_DIRS),)
	$(ECHO_COPYING_FRAMEWORK_DIRS)for d in $(_FRAMEWORK_DIRS); do \
		if [[ -d "$$d" ]]; then \
			rsync -a "$$d/" "$(_THEOS_SHARED_BUNDLE_BUILD_PATH)$(_THEOS_TARGET_BUNDLE_FRAMEWORK_SUBDIRECTORY)/" $(_THEOS_RSYNC_EXCLUDE_COMMANDLINE); \
		else \
			$(PRINT_FORMAT_WARNING) "Ignoring missing bundle framework directory $$d." >&2; \
		fi; \
	done$(ECHO_END)
endif
